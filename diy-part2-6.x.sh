#!/bin/bash
# ============================================================
# iStoreOS 24.10 (内核 6.6) ROCEOS K50S 云编译 DIY 脚本
# 基于官方 v22.03.7 DTS 适配，解决 5.10 → 6.6 节点差异
# 仓库: xiaobao1980/iStoreOS-RK35XX-24.10-NSY
# ============================================================

set -e

echo "==================== K50S 24.10 适配开始 ===================="

# --------------------------------------------------
# 步骤1: 通过内核 Patch 方式注入 DTS/DTSI（最可靠）
# --------------------------------------------------
PATCH_DIR="target/linux/rockchip/patches-6.6"
mkdir -p "${PATCH_DIR}"

# --- Patch 1: rk3568-roceos-k50s.dtsi ---
cat > "${PATCH_DIR}/999-rk3568-roceos-k50s-dtsi.patch" << 'PATCH1_EOF'
--- /dev/null
+++ b/arch/arm64/boot/dts/rockchip/rk3568-roceos-k50s.dtsi
@@ -0,0 +1,520 @@
...
echo "==> Patch 1 (DTSI) 已写入"

# --- Patch 2: rk3568-roceos-k50s.dts ---
cat > "${PATCH_DIR}/999-rk3568-roceos-k50s-dts.patch" << 'PATCH2_EOF'
--- /dev/null
+++ b/arch/arm64/boot/dts/rockchip/rk3568-roceos-k50s.dts
@@ -0,0 +1,95 @@
+// SPDX-License-Identifier: (GPL-2.0+ OR MIT)
+/*
+ * Copyright (c) 2020 Rockchip Electronics Co., Ltd.
